...
double  UBC(int Wins , int Visits , int ParentVisits){
    double UBC = (double)Wins/Visits + sqrt(2*log(ParentVisits)/Visits);
    cout<<UBC<<endl;
    return UBC;
}

//child class MonteCarlo of Solution 
class MonteCarlo : public Solution{
    private: 
    ...
    public: 
    void MCBot(vector<vector<int>> &board) {
    ...
    void MCPlayer(vector<vector<int>> &board){
        vector<vector<int>> dp(3 , vector<int>(3, INT_MIN)) ;
        //selection of moves :
        for(int i=0 ; i<3 ; i++){
            for(int j=0 ; j<3 ; j++){
